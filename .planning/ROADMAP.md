...
**Success Criteria** (what must be TRUE):
  ...
  1. Backfill of ≥1 year of MEXC USDT-perp OHLCV (1m/5m/15m/1h/4h/1d), funding, OI, and L2 top-20 snapshots completes against the Coinglass Hobbyist tier (~$35/mo per `project_data_tier_subscriptions.md`) without duplicates, with every derivatives row carrying an explicit `source` column distinguishing `mexc_native` from `coinglass_aggregate`. Note: 1m derivatives history is capped at ~6 days under the Hobbyist tier; 5m+/15m+/1h+ history is months-deep. The Coinglass Standard tier ($299/mo) upgrade is V2-DATA-01, deferred to Phase 2 EDA per CONTEXT.md D-37.
