...
- [ ] 00-02-PLAN.md — 8 pure-Pydantic v2 domain types (Candle, OrderBook, Funding, Liquidation, Signal, Order, Position, RiskLimits) + Hypothesis property tests on every invariant (EXEC-02 + RISK-02 structural)
- [ ] 00-03-PLAN.md — Per-service BaseAppSettings subclasses with SecretStr + safe_summary() + assert_no_trade_env_leaked() anti-leak guard + 4-layer secret-scan defense
